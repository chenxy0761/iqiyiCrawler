...
from iqiyiCrawler.items import IqiyiTVItem
...
class IqiyiSpider(scrapy.Spider):
    ...
    url = "http://www.iqiyi.com/v_19rrco0ypo.html"
    ...
    def parse(self, response):
        ...
        # print(self.gender_url.format(albumId=data.get("albumid")))
    ...
    def parse_gender(self, response):
        ...
        # print(albumid)
        gender = result.get("data").get("details")[0].get(albumid).get("gender")
        age = result.get("data").get("details")[0].get(albumid).get("age")
        data["gender"] = gender
        data["age"] = age
    ...
    def parse_index(self, response):
        ...
        if len(albumid) == 0:
            ...
            data = {"url": response.url, "status": status, "vip": str(vip), "vid": vid[0].strip("\""),
                    "tvid": tvid[0].strip("\""),
